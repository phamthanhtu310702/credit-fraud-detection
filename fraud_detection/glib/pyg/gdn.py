...
class graph_deviation_network(torch.nn.Module):
    def __init__(self, cfg, device, hidden_dim):
        super(graph_deviation_network, self).__init__()
        self.net = AnomalyLayer(hidden_dim)
        self.memory_size = cfg['memory_size'] # 5000 # 4000
        self.sample_size = cfg['sample_size'] # 2000 # 1000
        self.device = device
    ...
    def dev_diff(self, y_prediction, current_time):
        index = torch.LongTensor(random.sample(range(self.memory_size), self.sample_size)).to(self.device)
        ref = torch.index_select(self.memory, 0, index)
        ref_time = torch.index_select(self.time_memory, 0, index)
        lambda_t = 1 / 3600
        time_diff = (torch.abs(torch.from_numpy(current_time).reshape([-1, 1]).to(self.device) - ref_time.reshape([1, -1])))
        w_time = 1 / (torch.log(lambda_t * time_diff + 1) + 1)
    ...
    def forward(self, x, time, label=None):
        ...
        if self.training:
            ...
            record_time_mem = (torch.from_numpy(time).clone().detach())[label <= 0]
